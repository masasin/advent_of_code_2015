...
def part_one():
    sues = []
    with open("inputs/day_16_input.txt") as fin:
        for line in fin.readlines():
            sues.append(parse(line))
    for i, sue in enumerate(sues):
        if matches(sue, results):
            print("{} matches".format(i+1))
